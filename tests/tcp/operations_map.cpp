...
void op_func1(boost::system::error_code ec, typename es::operation<>::package_view_type view) { ++op_func_test; }
void op_func2(boost::system::error_code ec, typename es::operation<>::package_view_type view) { ++op_func_test; }

void op_func_bind1(boost::system::error_code ec, typename es::operation<>::package_view_type view, int& op_func_bind_test) { ++op_func_bind_test; }
void op_func_bind2(boost::system::error_code ec, typename es::operation<>::package_view_type view, int& op_func_bind_test) { ++op_func_bind_test; }
...
TEST_CASE("operations_map has random access by guid to type erased operations", "[operations_map]")
{
	...
	SECTION("operations_map can register and call lambdas")
	{
		auto key1 = es::guid();
		int test1 = 0;
		auto token1 = [&test1](boost::system::error_code, package_view_type) { test1 = 10; };
		operation_type op1(std::move(token1));
		ops_manager.register_op(key1, std::move(op1));
		auto key2 = es::guid();
		int test2 = 0;
		ops_manager.register_op(key2, operation_type([&test2](boost::system::error_code ec, package_view_type view) -> void { test2 = 6; }));

		ops_manager[key1](boost::system::error_code{}, package_view_type{ nullptr, 0 });
		ops_manager[key2](boost::system::error_code{}, package_view_type{ nullptr, 0 });
